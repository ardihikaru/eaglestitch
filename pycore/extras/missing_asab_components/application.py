"""
This class file was taken from the original Application class file of ASAB Framework,
and was modified accordingly
Original file: https://github.com/TeskaLabs/asab/blob/v21.01-rc2/asab/application.py
"""
import asab
import logging
import asyncio
...
from extras.logging.log import Logging
from extras.logging.task import TaskService
from extras.missing_asab_components.docker import running_in_docker
from asab.log import LOG_NOTICE
from asab.log import _loop_exception_handler
...
class Application(metaclass=Singleton):
	...
	def __init__(self, args=None):
		...
		# Setup logging
		# Here, we override with our custom Logging class!
		self.Logging = Logging(self)

		# Configure the event loop
		self.Loop.set_exception_handler(_loop_exception_handler)
		if asab.Config["logging"].getboolean("verbose"):
		...
		from asab.pubsub import PubSub
		...
		try:
			...
			for cycle_no in itertools.count(1):
				try:
					...
					if (cycle_no % 3) == 0:
						self.PubSub.publish("Application.tick/3!")
					if (cycle_no % 5) == 0:
						self.PubSub.publish("Application.tick/5!")
					if (cycle_no % 10) == 0:
